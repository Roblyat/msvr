...
int main()
{
    ...
    while (key != 'd')
    {
        ...
        siftCamera.matchDescriptors(storage.descriptors, storage.cameraDescriptors, 
            storage.goodMatches, storage.keypoints, storage.cameraKeypoints, storage.img_with_keypoints, storage.camera_img_with_keypoints);

        cv::imshow("Camera Features", storage.camera_img_with_keypoints);
